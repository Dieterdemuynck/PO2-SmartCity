...
Er werden twee poorten va, de my Rio met elkaar verbonden en als de microcontroller gekanteld werd, moest een signaal verstuurd worden van de ene poort naar de andere.
...
\begin{figure}
	...
	\end{tikzpicture}
	\caption{Het basisidee van het programma van de reflectiesensor, bepaald door de waarde 0}
\end{figure}
...
\begin{figure}
	...
	\end{tikzpicture}
	\caption{Het basisidee van het programma van de kleurensensor, bepaald door de waarde 1}
\end{figure}

Nog een interessante opmerking over onderdeel \ref{waarde2} van het basisprogramma. TO DO: nog verder schrijven

\begin{figure}
	\label{waarde2}
	\hspace*{-2.5cm}
	\begin{tikzpicture}[node distance = 4.5cm, auto]
		\node [draw, trapezium, fill=gray!20, trapezium left angle = 65, trapezium right angle = 115](2){Waarde 2};
		...
		\node [draw, trapezium, fill=gray!20, trapezium left angle = 65, trapezium right angle = 115, below right of=JaGevonden] (waarde0) {Waarde 0 \ref{reflectiesensorValueZero}};
		\node [draw, trapezium, fill=gray!20, trapezium left angle = 65, trapezium right angle = 115, above right of=JaGevonden] (opnieuw) {Terug naar waarde 2};
	...
	\end{tikzpicture}
	\caption{Het programma om het autootje te laten draaien in een notendop.}
\end{figure}
...
\section{Besluit}
Er is duidelijk te zien dat er veel opzoekingswerk vereist is. Er moet namelijk gezorgd worden dat alle onderdelen bij elkaar passen: geen te grote wielen, onderdelen die niet te veel wegen… Dit het belangrijkste onderdeel van het ontwerpproces.\\
Voor het implementeren is er ook heel wat opzoekingswerk vereist. Daarom is het belangrijk dat dit werk vroeg genoeg wordt ingepland, zodat men genoeg tijd zal hebben om effectief te programmeren. Eens er begonnen wordt met het programmeren is het ten sterkste aangeraden om over de onderdelen van de auto te beschikken. Zo zal er geëxperimenteerd kunnen worden. Om een beter beeld te krijgen over de programma’s die moeten geïmplementeerd worden, is het handig om vooraf een flowchart te maken. Aan  de hand van deze flowcharts zou het mogelijk moeten zijn om een programma uit te schrijven.\\
Om de auto te verbeteren, is het aangeraden om meer gewicht op de voorkant te laten rusten, zodat het kogelwiel achteraan minder wrijving zou ondervinden. Het gebruik van krachtigere wielen kan ook voor een oplossing zorgen. Ook kon men besluiten dat voor het interpreteren van een verkeerslicht, het gebruik van een camera misschien meer ideaal zou geweest zijn. Een nadeel hiervan is het bijkomend gewicht.
In het geval dat we meer tijd gehad zouden hebben, zouden de programma’s meer getest kunnen worden en zouden de programma’s nog verbeterd kunnen worden.
